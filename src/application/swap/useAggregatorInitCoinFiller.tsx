...
import { BRRRMint } from '@/application/token/wellknownToken.config'
...
export default function useAggregatorInitCoinFiller() {
  ...
  useEffect(() => {
    ...
    if (!coin2 && coin1?.mintString !== toPubString(BRRRMint) && !hasOutputCurrency) {
      useAggregator.setState({ coin2: getToken(BRRRMint) })
    }
